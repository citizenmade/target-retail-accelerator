  ...
  <div id="content_wrap">
    ...
    <form method="post" id="the-theme-options">
      ...
      <div class="info top-info">
        ...
        <input type="submit" value="<?php _e('Save All Changes','mobilize') ?>" class="button-framework save-options" name="submit" />
        
        <?php if ( $this->has_xml && $this->show_docs == false ) { ?>
        <input type="submit" value="<?php _e('Reload XML','mobilize') ?>" class="button-framework reload-options" name="reload" style="margin-right:10px;" />
        <?php } ?>
        <?php
          ...
          { 
            ...
            $selected = '';
  	        ...
  	        echo '<option'.$selected.'>'.trim( $key ).'</option>';
       		...
       		echo '</select>';
       		?>
       		<input type="submit" value="<?php _e('Activate Layout','mobilize') ?>" class="button-framework user-activate-layout" name="user-activate-layout" style="margin-right:10px;" />
       		<?php
       		echo '</div>';
     		}
        ?>
    ...
    <form method="post" id="flv-reinstall-table">
        <input type="submit" value="<?php _e('Reinstall Options Table','mobilize') ?>" class="button-secondary" name="flv-reinstall-table"/>
    </form>
    <form method="post" id="flv-reinstall-opts">
        <input type="submit" value="<?php _e('Reinstall All Options','mobilize') ?>" onclick="var con = confirm('Are you sure you want to reinstall the options? Previous options will be deleted.'); if(con){}else{ return false; }" class="button-secondary" name="flv-reinstall-opts"/>
    </form>
<!-- [END] framework_wrap -->