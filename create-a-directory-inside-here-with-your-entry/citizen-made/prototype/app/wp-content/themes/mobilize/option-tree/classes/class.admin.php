...
class OT_Admin
{
  ...
  private $has_layout;
  private $show_docs;
  public $the_option='mobilize_options';
  ...
  function __construct() 
  {
    ...
    $this->table_name = $table_prefix . 'custom_option_tree_mobilize';
  ...
  function option_tree_table( $type = '')
  {
    if ( $type == 'create' ) 
    {
      $sql = "CREATE TABLE {$this->table_name} (
        ...
        item_options VARCHAR(5000) DEFAULT NULL,
  ...
  function option_tree_load_theme_files() 
  {
    ...
    if ( $this->has_data == true && !get_option( $this->the_option ) )
    {
      ...
      {
        // create new options
        add_option($this->the_option, $new_options);
      }
    }
   ...
   * @return void
   */
  function option_tree_default_data($reinstall_table="off", $reinstall_opts="off")
  {
      global $wpdb;
    ...
    $ot_xml_file = get_template_directory(). "/option-data/theme-options.xml";
    //echo $ot_xml_file;
    ...
    $new_installation = $wpdb->get_var( "show tables like '$this->table_name'" ) != $this->table_name;
    if($new_installation || $reinstall_table=='on'){
    ...
    $auxopt = get_option( $this->the_option );

    //if($auxopt=='' || $arg1=='on'){
    
    if($auxopt=='' || $reinstall_opts=='on'){
    $new_options_data_string=file_get_contents(get_template_directory(). "/option-data/options.txt");
    $new_options = unserialize($new_options_data_string);
    replace_in_matrix('http://localhost/wordpress3/wp-content/themes/mobilize/', THEME_URL, $new_options);
    update_option($this->the_option, $new_options);
    }
  ...
  function option_tree_admin() 
  {
    ...
    // load options array
  	$settings = get_option( $this->the_option );
    ...
    $option_tree_options = add_theme_page( 'Theme Options', 'Theme Options', 'manage_options', 'mobilize_settings', array( $this, 'option_tree_options_page' ) );
 //   $option_tree_docs = add_theme_page( 'Theme Options', 'Editor', 'manage_options', 'option_tree_docs', array( $this, 'option_tree_docs_page' ) );
 //   $option_tree_settings = add_theme_page(  'OptionTree', 'OptionTree Settings', 'manage_options', 'option_tree_settings', array( $this, 'option_tree_settings_page' ) );
          
          
   //   add_menu_page( 'FreasIdeas Settings', 'FreshIdeas Settings', 'manage_options', $this->the_option, array( $this, 'option_tree_options_page' ), $icon);
   //   $option_tree_options = add_submenu_page( $this->the_option, 'OptionTree', 'Theme Options', 'manage_options', $this->the_option, array( $this, 'option_tree_options_page' ) );
   //   $option_tree_docs = add_submenu_page( 'OptionTree', 'Options treee', 'Documentation', 'manage_options', 'option_tree_docs', array( $this, 'option_tree_docs_page' ) );
   //   $option_tree_settings = add_submenu_page( $this->the_option, 'OptionTree', 'Developer Settings', 'manage_options', 'option_tree_settings', array( $this, 'option_tree_settings_page' ) );
 ...
 //     add_action( "admin_print_styles-$option_tree_docs", array( $this, 'option_tree_load' ) );
  ...
  function option_tree_array_save() 
  {
    ...
    update_option( $this->the_option, $new_settings );
    ...
    if ( isset($_GET['action']) && $_GET['action'] == 'upload' ) 
    {
      ...
      else 
      {
        ...
        if ( preg_match( "/(.xml)$/i", $_FILES["import"]['name'] ) ) 
        {
          ...
          // success redirect
          header("Location: admin.php?page=option_tree");
          die();
        }
        // fail
        else
  ...
  function option_tree_update_export_data() 
  {
    ...
    $updated = base64_encode( serialize( get_option( $this->the_option ) ) );
  ...
  function option_tree_save_layout() 
  {
    ...
    $options = get_option( $this->the_option );
  ...
  function create_option_post() 
  {
    ...
    if ( isset($_GET['page']) && $_GET['page'] == $this->the_option ) 
    {  
  ...
  function option_tree_notice_post_locked( $post_id ) 
  {
    ...
    $last_user_name = $last_user ? $last_user->display_name : __('Somebody','mobilize');
    $the_page = ( $_GET['page'] == $this->the_option ) ? __('Theme Options','mobilize') : __('Settings','mobilize');
    
    $message = sprintf( __( 'Warning: %s is currently editing the %s.' ,'mobilize'), esc_html( $last_user_name ), $the_page );
